...
static bool pt_offline_mode = false;
static struct dentry *pt_mode_dentry = NULL;
...
static ssize_t
pt_mode_write(struct file *filp, const char __user *buf,
		size_t count, loff_t *ppos)
{
	char mode;
	...
	pt_close_logfile();
	pt_offline_mode = false;
	workqueue_set_max_active(pt_wq, PT_NUM_WORKERS);

	pt_print("online mode\n");

	return 1;
}

static const struct file_operations pt_mode_fops = {
	.write = pt_mode_write,
};

static int pt_mode_setup(void)
{
	pt_mode_dentry = debugfs_create_file("pt_mode",
			0600, NULL, NULL, &pt_mode_fops);
	if (!pt_mode_dentry) {
		pt_print("unable to create pt_mode\n");
		return -ENOMEM;
	}
...
static void pt_mode_destroy(void)
{
	if (pt_mode_dentry)
		debugfs_remove(pt_mode_dentry);
}
...
void pt_on_clone(struct task_struct *child)
{
	...
	if (child->tgid == child->pid) {
		...
		pt_pause();
		pt_flush_trace(child_topa, pt_offline_mode);
		pt_resume();
	}
