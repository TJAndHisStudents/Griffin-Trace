...
Griffin is a kernel patch that relies on specific Intel Processor Trace (PT) hardware. The Griffin kernel will only work on an Intel processor that includes Processor Trace. Modern Intel processors include this hardware.

For generating policies for Griffin to monitor and enforce forward edges, you'll need LLVM 3.7 with the ```opt``` package available. Then you can use the compiler pass located in
```tools/pt/extras/fpt-llvm-pass/```.
...
Monitoring and enforcing forward edges require generating an enforcement policy that Griffin can use during program runtime to enforce legal forward edges. To generate the policies needed, you'll need to use the Function Pointer Trace (FPT) tool and install it to LLVM. FPT is inside ```tools/pt/extras/fpt-llvm-pass/```.

Next you will need to use the Python script ```generate_policy.py``` to generate the policy from the FPT file and the executable. This can be found in ```tools/pt/extras/fpt-llvm-pass/```.
